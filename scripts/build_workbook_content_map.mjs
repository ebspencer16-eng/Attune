...
const part2Section = [
  ...
  sectionTable([
    ...
    { what: '"What {Partner} should know about {You}" page header (cross-type only)',
      source: 'Their names',
      notes: 'Cross-type couples get two pages, one per partner. Each is one-directional guidance keyed to that partner\'s individual type.' },
    { what: '"How you two should approach specific situations" page header (same-type only)',
      source: 'Universal',
      notes: 'Same-type couples (WW, XX, YY, ZZ) get one shared section instead. Both partners share the wiring, so guidance is symmetric.' },
    ...
    { what: 'Each moment, "What\'s happening" row',
      source: 'Couple-type-specific',
      notes: 'Four versions written (W, X, Y, Z). Cross-type: the subject partner\'s letter picks the version. Same-type: the shared letter picks the version, and the row is framed as "for both of you."' },
    { what: 'Each moment, "What NOT to do" / "Where two same-type partners get stuck"',
      source: 'Couple-type-specific',
      notes: 'Four versions (W, X, Y, Z). Cross-type: the natural but wrong move for the other partner. Same-type: the trap two same-type partners fall into when the shared wiring mirrors instead of balances.' },
    { what: 'Each moment, "What works"',
      source: 'Couple-type-specific',
      notes: 'Four versions (W, X, Y, Z). Cross-type: the specific action that lands. Same-type: how to break the shared default.' },
    { what: 'Each moment, "Phrase that lands" / "A cue either of you can use"',
      source: 'Couple-type-specific',
      notes: 'Four versions (W, X, Y, Z). Cross-type: a literal line one partner says to the other. Same-type: a cue either partner can use to break the loop.' },
  ]),
];
...
const summaryPage = [
  ...
  ...[
    ...
    { color: PURPLE, label: 'Couple-type-specific', desc: 'One of 10 versions per block. Drives the type name everywhere it appears, the Working Knowledge scenes (24 cross-type + same-type variants), the gap analysis text on dimension pages, the conversation prompts chosen, and the Reference Card phrase.' },
