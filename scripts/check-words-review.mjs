#!/usr/bin/env node
/**
 * WORDS-REVIEW.md says what api/_words.js says.
 *
 * ── THE PROMISE ───────────────────────────────────────────────────────────
...
if (before === null) {
  console.error('[check-words-review] WORDS-REVIEW.md is missing. Run'
    + ' `node scripts/build-words-review.mjs`.');
  process.exit(1);
}

execFileSync(process.execPath, [`${ROOT}scripts/build-words-review.mjs`], { stdio: 'pipe' });
const after = readFileSync(OUT, 'utf8');

if (before !== after) {
  console.error('[check-words-review] WORDS-REVIEW.md was out of date with'
    + ' api/_words.js and has been regenerated. Commit it.');
  process.exit(1);
}
console.log('[check-words-review] the review document is the word list, regenerated and identical.');
