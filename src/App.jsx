...
// historical and stable. calcDimScores, retake snapshots, and the workbook
// generator all reference these IDs by name.
// PERSONALITY_QUESTIONS now lives in api/_questions.js (single source of truth).
...
const NEAR_AXIS_PROSE = NEAR_AXIS_PROSE_SHARED;

// Moved to api/_couple-types.js so /api/results can return it and the app can
// render it. Imported at the top of this file. One copy, server-owned.
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      <svg viewBox={`0 0 ${VB} ${cy1 + 42}`} style={{ width: "100%", display: "block", overflow: "visible" }} aria-label="Couple map showing partner positions">
        ...
        <text x={midX} y={cy0 - 16} textAnchor="middle" fontSize="11" fontWeight="700" fill={axisLabelColor} fontFamily="Arial" letterSpacing="2">ENGAGE</text>
        <text x={midX} y={cy1 + 30} textAnchor="middle" fontSize="11" fontWeight="700" fill={axisLabelColor} fontFamily="Arial" letterSpacing="2">WITHDRAW</text>
        {/* Horizontal axis: Open (left) / Guarded (right) — rotated to run along each side */}
        <text x={cx0 - 14} y={midY} textAnchor="middle" fontSize="11" fontWeight="700" fill={axisLabelColor} fontFamily="Arial" letterSpacing="2" transform={`rotate(-90 ${cx0 - 14} ${midY})`}>OPEN</text>
        <text x={cx1 + 14} y={midY} textAnchor="middle" fontSize="11" fontWeight="700" fill={axisLabelColor} fontFamily="Arial" letterSpacing="2" transform={`rotate(90 ${cx1 + 14} ${midY})`}>GUARDED</text>
    ...
    <div style={{ marginTop: "1rem", background: "rgba(255,255,255,0.06)", borderRadius: 14, padding: "1.2rem 1.5rem", border: "1px solid rgba(255,255,255,0.14)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1.6rem" }}>
        {rows.map(({ q, text, lo, hi, a, b }) => {
          ...
          return (
            <div key={q.id}>
              <div style={{ fontSize: "0.78rem", color: "rgba(255,255,255,0.92)", fontFamily: BFONT, lineHeight: 1.5, marginBottom: "0.85rem", fontWeight: 500 }}>{text}</div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.7rem" }}>
                <span style={{ fontSize: "0.58rem", color: "rgba(255,255,255,0.5)", fontFamily: BFONT, lineHeight: 1.3, flex: "0 0 27%", textAlign: "right" }}>{lo}</span>
                ...
                <span style={{ fontSize: "0.58rem", color: "rgba(255,255,255,0.5)", fontFamily: BFONT, lineHeight: 1.3, flex: "0 0 27%", textAlign: "left" }}>{hi}</span>
              </div>
            </div>
          );
        })}
...
const EXP_CAT_STARTERS = EXP_CAT_STARTERS_SHARED;
...
function ExpectationsResults({ myAnswers, partnerAnswers, userName, partnerName, forcedSection, noSideNav = false, onGoWhatComesNext, onGoBack, onExternalGo, coupleTypeCode = null, coupleTypeName = null, coupleTypeColor = "#1B5FE8" }) {
  ...
  if (convoMatch) {
    ...
    const catNarration = CATEGORY_INTRO;
...
// reflectionActionTitle and deriveAnniversaryInsights moved to
// api/_lib/reflection-insights.js so the app can render the same plan.
...
function UnifiedResults({ ex1Answers, partnerEx1, ex2Answers, partnerEx2, ex3Answers, partnerEx3, ex2AnswersPrior = null, ex2PriorAt = null, hasAnniversary, userName, partnerName, initialSection, onSectionChange = null, isMobile = false, portrait = null, hasChecklist = false, hasBudget = false, hasWorkbook = false, hasIntimacy = false, intimacyAnswers = null, partnerIntimacy = null, hasConflict = false, conflictAnswers = null, partnerConflict = null, intimacyVariant = 'premarital', onNavigateTool = null, userPronouns = "", partnerPronouns = "", isBetaTester = false }) {
  ...
  if (section.startsWith("reflection") && hasAnniversary) {
    ...
    // ── REFLECTION INSIGHTS ───────────────────────────────────────────────────
    // ── HOW YOU EACH RATED ────────────────────────────────────────────────────
    // Everything with a fixed answer: the scales, the admiration pick, the
    // priority ranking. Shown as comparisons rather than described in prose.
              ...
              <div style={{ marginBottom: "1.5rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                  {[[userName, partnerName, mine.a8, "#E8673A"], [partnerName, userName, theirs.a8, "#1B5FE8"]].map(([from, about, val, col]) => (
                    <div key={from} style={{ background: "white", border: `1.5px solid ${col}30`, borderRadius: 14, padding: "1.1rem 1.25rem", borderTop: `3px solid ${col}` }}>
                      <div style={{ fontSize: "0.58rem", letterSpacing: "0.14em", textTransform: "uppercase", color: col, fontWeight: 700, fontFamily: BFONT, marginBottom: "0.5rem" }}>{from} admires</div>
                      <div style={{ fontFamily: HFONT, fontSize: "1.35rem", fontWeight: 700, color: C.ink, lineHeight: 1.1 }}>{admiredNoun(val)}</div>
                      <div style={{ fontSize: "0.7rem", color: C.muted, fontFamily: BFONT, marginTop: "0.3rem" }}>in {about}</div>
                    </div>
                  ))}
                </div>
                {mine.a8 === theirs.a8 && (
                  <div style={{ marginTop: "0.6rem", background: "#EDFAF5", border: "1px solid #10b98130", borderRadius: 10, padding: "0.6rem 0.9rem", fontSize: "0.76rem", color: "#047857", fontFamily: BFONT, fontWeight: 600 }}>
    ...
    if (section === "reflection-story") {
      // The previous synthesis note guessed a theme from keywords and could
  ...
  if (conflictListed && section.startsWith("conflict-")) {
    ...
    const PatternRow = ({ p }) => {
      ...
      return (
        <div key={p.key} style={{ padding: "1rem 0", borderTop: `1px solid ${C.stone}` }}>
          ...
          {advice && (
            <div style={{ marginTop: "0.8rem", background: "#F4F7FF", borderRadius: 8, padding: "0.75rem 0.95rem" }}>
              {/* Physical Intimacy's plan is a detail page, so the label stays.
                  Which of the two it reads depends on the frequency band, and
                  that difference is the point of having it. */}
    ...
    if (section === "conflict-snapshot") {
      // Three columns: the question on the left, then each person's answer.
      // With the label above the pair, the eye had to travel down and back up
      // to connect a question to its answers.
      const COLS = isMobile ? "1fr" : "minmax(0,1.3fr) minmax(0,1fr) minmax(0,1fr)";
      return (
        <Layout accent={BLUE}>
              ...
              <Card>
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "1.75rem" }}>
                  {[[partnerName, userName, conflictMine.repairRanking],
                    [userName, partnerName, conflictPairing.b.repairRanking]].map(([actor, wants, ranking]) => (
                    <div key={wants}>
                      <div style={{ fontSize: "0.62rem", letterSpacing: "0.14em", textTransform: "uppercase", color: C.muted, fontWeight: 700, fontFamily: BFONT, marginBottom: "0.7rem", lineHeight: 1.4 }}>
                        What {wants} wants from {actor}, in order
                      </div>
    ...
    // ── Results at a glance ──────────────────────────────────────────────
    // Coloured ground, matching the reflection glance page. Q1 is the
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.7rem" }}>
              {flagged.length > 0 ? flagged.map(p => (
                <div key={p.key} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.14)", borderRadius: 14, padding: "1rem 1.2rem" }}>
                  ...
                  <p style={{ fontSize: "0.82rem", color: "rgba(255,255,255,0.85)", fontFamily: BFONT, lineHeight: 1.65, margin: 0 }}>{interp(PATTERN_ACTIONS[p.key]?.body)}</p>
                </div>
              )) : (
                <div style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.14)", borderRadius: 14, padding: "1rem 1.2rem" }}>
                  <div style={{ fontSize: "0.88rem", fontWeight: 700, color: "white", fontFamily: BFONT, marginBottom: "0.25rem" }}>{NO_ACTION_NEEDED.title}</div>
                  <p style={{ fontSize: "0.82rem", color: "rgba(255,255,255,0.85)", fontFamily: BFONT, lineHeight: 1.65, margin: 0 }}>{interp(NO_ACTION_NEEDED.body)}</p>
                </div>
              )}
            </div>
          </div>
          ...
          if (myDone && partnerDone) {
            ...
            return (
              <div style={{ maxWidth: 480, margin: "0 auto", padding: "3rem 1rem", textAlign: "center" }}>
                ...
                <button onClick={() => { setActiveResult("intimacy-overview"); setHighlightsSeen(true); setView("results"); }}
                  style={{ background: "#B5546E", color: "white", border: "none", borderRadius: 12, padding: "0.9rem 1.75rem", fontSize: "0.9rem", fontWeight: 600, fontFamily: BFONT, cursor: "pointer" }}>
                  View intimacy results →
                </button>
              </div>
            );
          }
