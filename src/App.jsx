...
function PersonalityResults({ myAnswers, partnerAnswers, userName, partnerName, coupleType, noSideNav = false, externalStep, onExternalGo, onGoExpectations }) {
    ...
    <MaybeNav noSideNav={noSideNav} navItems={personalityNavItems} currentStep={step} onGo={go} accent="#E8673A">
      ...
      <div style={{ color: "white" }}>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: "0.85rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem" }}>
            {/* Dimension detail pages first */}
            {sortedFeedback.map((f, idx) => {
              const m = DIM_META[f.dim];
              const isAligned = f.gap <= 1;
              return (
                <div key={f.dim} onClick={() => go(idx + 1)}
                  style={{ display: "flex", alignItems: "center", gap: "0.65rem", padding: "0.4rem 0.75rem", borderRadius: 8, background: "rgba(255,255,255,0.04)", cursor: "pointer", transition: "all 0.12s", border: "1px solid rgba(255,255,255,0.06)" }}
                  onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.09)"}
                  onMouseLeave={e => e.currentTarget.style.background = "rgba(255,255,255,0.04)"}>
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: m.color, flexShrink: 0 }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: "0.74rem", color: "rgba(255,255,255,0.72)", fontFamily: BFONT }}>{m.label}</div>
                  </div>
                  <span style={{ fontSize: "0.55rem", color: isAligned ? "#10b981" : "rgba(232,103,58,0.7)", fontFamily: BFONT, fontWeight: 600, flexShrink: 0 }}>{isAligned ? "aligned" : "explore →"}</span>
                </div>
              );
            })}
            {/* Divider */}
            <div style={{ height: "1px", background: "rgba(255,255,255,0.08)", margin: "0.2rem 0" }} />
            {/* Profiles + action plan */}
            {[
              { label: userName + "'s profile", sub: "How " + userName + " is wired", s: orderedDims.length + 1 },
              { label: partnerName + "'s profile", sub: "How " + partnerName + " is wired", s: orderedDims.length + 2 },
              { label: "Communication Action Plan", sub: "Practices built from your results", s: orderedDims.length + 3 },
            ].map(({ label, sub, s }) => (
  ...
  if (step === orderedDims.length + 1 || step === orderedDims.length + 2) {
    ...
    // Scores must be declared first — used by makeIndivProfile below
    const personScores = isMyPage ? myS : partS;
    const partnerScores = isMyPage ? partS : myS;
    const makeIndivProfile = (scores) => {
      const top3 = DIMS.filter(d => scores[d] !== undefined).sort((a,b) => Math.abs((scores[b]||3)-3) - Math.abs((scores[a]||3)-3)).slice(0,3);
      return top3.map(d => {
        const m = DIM_META[d]; if (!m) return null;
        const end = scores[d] < 3 ? m.ends[0] : m.ends[1];
        const pEnd = scores[d] < 3 ? m.ends[1] : m.ends[0];
        return { dim: m.label, end, pEnd, color: m.color, score: scores[d] };
      }).filter(Boolean);
    };
    const myDims = makeIndivProfile(personScores);
    const energyGap = Math.abs((personScores.energy||3)-(partnerScores.energy||3));
    const conflictGap = Math.abs((personScores.conflict||3)-(partnerScores.conflict||3));
    const practice = (() => {
      const tips = [];
      if (energyGap > 1.5) tips.push(isMyPage ? "Your partner's energy style differs from yours in ways that will show up regularly. When one of you needs to recharge and the other wants connection, that gap is almost never about the relationship." : "Your partner recharges differently than you do. What looks like withdrawal is usually restoration. What looks like neediness is usually seeking connection.");
      if (conflictGap > 1.5) tips.push(isMyPage ? "You and your partner have different conflict clocks. When something needs addressing, one of you will be ready before the other. That mismatch is worth naming explicitly." : "Your conflict timing differs. The person who needs space isn't avoiding, and the person who needs resolution isn't overreacting. Naming your respective needs before the hard moment makes this much smoother.");
      if (tips.length === 0) tips.push("Your communication styles are closely matched in several key areas. The thing to watch is staying curious about subtle differences rather than assuming full alignment.");
      return tips.join(" ");
    })();
    const myProfile = { tag: myDims.map(d => d.end + " in " + d.dim.toLowerCase()).join(". ") + ".", blind: "Communication style is always evolving, and your strongest orientations sometimes show up as blind spots." };
      ...
      <ResultsSlide bg={pageBg}>
        ...
        <div style={{ color: "white" }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 16, padding: "1.25rem", marginBottom: "0.85rem", border: ("1px solid " + (pageColor) + "20") }}>
            ...
            <p style={{ fontSize: "0.82rem", color: "rgba(255,255,255,0.72)", fontFamily: BFONT, fontWeight: 300, marginBottom: "0.85rem", fontStyle: "italic" }}>{myProfile.tag}</p>
            {myDims.map((d, i) => (
              <div key={i} style={{ display: "flex", gap: "0.65rem", marginBottom: i < myDims.length - 1 ? "0.55rem" : 0 }}>
                <span style={{ color: d.color, flexShrink: 0, fontSize: "0.9rem" }}>→</span>
                <span style={{ fontSize: "0.86rem", color: "rgba(255,255,255,0.92)", fontFamily: BFONT, lineHeight: 1.55 }}>{d.end + " in " + d.dim.toLowerCase()}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 16, padding: "1.1rem 1.25rem", marginBottom: "0.85rem", borderLeft: "3px solid rgba(255,152,0,0.6)" }}>
            ...
            <p style={{ fontSize: "0.86rem", color: "rgba(255,255,255,0.88)", fontFamily: BFONT, fontWeight: 300, lineHeight: 1.7, margin: 0 }}>{myProfile.blind}</p>
          </div>

          {/* Standout traits */}
          {distinctDims.length > 0 && (
            <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 16, padding: "1.1rem 1.25rem", marginBottom: "0.85rem" }}>
              <div style={{ fontSize: "0.62rem", textTransform: "uppercase", letterSpacing: "0.15em", color: "rgba(255,255,255,0.75)", fontWeight: 700, marginBottom: "0.65rem", fontFamily: BFONT }}>Your strongest orientations</div>
              {distinctDims.map(d => {
                const m = DIM_META[d];
          ...
          {/* With [partner] specifically, at the bottom, most actionable */}
          <div style={{ background: (pageColor + "18"), borderRadius: 16, padding: "1.25rem", border: ("1px solid " + (pageColor) + "40") }}>
...
function UnifiedResults({ ex1Answers, partnerEx1, ex2Answers, partnerEx2, ex3Answers, partnerEx3, hasAnniversary, userName, partnerName, initialSection, isMobile = false, portrait = null, hasChecklist = false, hasBudget = false, hasLMFT = false, onNavigateTool = null, userPronouns = "", partnerPronouns = "" }) {
  ...
  const allPages = [
    "highlights",
    "couple-type", "couple-map", "summary",
    "comm-overview",
  ...
  const Layout = ({ children, accent, noPrevNext = false }) => (
    <div>
      ...
      <div style={{ display: "flex", gap: "0", alignItems: "flex-start" }}>
        <div className="desktop-sidebar" style={{ position: "sticky", top: 0, paddingTop: "1.25rem" }}><Sidebar /></div>
        <div style={{ flex: 1, minWidth: 0 }}>
          {children}
          {!noPrevNext && <PrevNext />}
        </div>
        ...
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          ...
          <div style={{ marginBottom: "1.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.65rem" }}>
              <NavActionLink onClick={() => go("comm-plan")} bg="linear-gradient(135deg,#FFF5F2,#FFE8E0)" border={`1.5px solid rgba(232,103,58,0.25)`} icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>} title="Communication Action Plan" sub="Practices drawn from your communication results" accentColor={C.orange} />
          ...
          {/* ── IN PRACTICE (moved to bottom) ── */}
          <a href="/practice" style={{ display: "flex", alignItems: "center", gap: "1rem", background: C.warm, border: `1.5px solid ${C.stone}`, borderRadius: 16, padding: "1.1rem 1.4rem", textDecoration: "none", marginBottom: "0.5rem" }}>
            <div style={{ width: 38, height: 38, borderRadius: 10, background: C.orange + "18", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={C.orange} strokeWidth="2" strokeLinecap="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>
            </div>
            <div>
              <div style={{ fontSize: "0.82rem", fontWeight: 700, color: C.ink, fontFamily: BFONT, marginBottom: "0.15rem" }}>In Practice — Attune's resource library</div>
              <div style={{ fontSize: "0.72rem", color: C.muted, fontFamily: BFONT }}>Guides on communication, conflict, and growing together</div>
            </div>
            <div style={{ marginLeft: "auto", color: C.muted, fontSize: "1rem" }}>→</div>
          </a>
  ...
  const content = (
    <>
      ...
      {!inline && <div style={{ height: 52, background: "rgba(20,16,40,0.97)", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 1.5rem", flexShrink: 0 }}>
        <span style={{ fontSize: "0.72rem", color: "rgba(255,255,255,0.35)", fontFamily: BFONT, letterSpacing: "0.06em" }}>Your results</span>
...
function AuthModal({ mode, onClose, onSuccess }) {
  ...
  const [form, setForm] = useState({ name: "", pronouns: "", partnerName: "", partnerPronouns: "", partnerEmail: "", email: "", password: "", emailOptIn: true });
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}
      ...
      <div style={{ background: "#FFFDF9", borderRadius: 22, padding: "2rem 2rem 1.75rem", width: "100%", maxWidth: 440, boxShadow: "0 32px 80px rgba(0,0,0,0.28)", position: "relative" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "1.5rem" }}>
          <svg width="28" height="20" viewBox="0 0 103 76" fill="none"><defs><linearGradient id="am1" x1="0" y1="0" x2="103" y2="76" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#E8673A"/><stop offset="100%" stopColor="#1B5FE8"/></linearGradient></defs><path d="M14,4 L44,4 A9,9 0 0,1 53,13 L53,42 A9,9 0 0,1 44,51 L20,51 L6,61 L11,51 A6,6 0 0,1 5,45 L5,13 A9,9 0 0,1 14,4 Z" fill="url(#am1)"/><path d="M22 11 C20 8.5 16.5 5 11.5 5 C5.5 5 2 9.5 2 14.5 C2 23 11 30 22 40 C33 30 42 23 42 14.5 C42 9.5 38.5 5 32.5 5 C27.5 5 24 8.5 22 11 Z" fill="white" opacity="0.93" transform="translate(13.16,11.3) scale(0.72)"/><path d="M89,14 L59,14 A9,9 0 0,0 50,23 L50,52 A9,9 0 0,0 59,61 L83,61 L97,71 L92,61 A6,6 0 0,0 98,55 L98,23 A9,9 0 0,0 89,14 Z" fill="none" stroke="url(#am1)" strokeWidth="2.2" strokeLinejoin="round"/><path d="M22 11 C20 8.5 16.5 5 11.5 5 C5.5 5 2 9.5 2 14.5 C2 23 11 30 22 40 C33 30 42 23 42 14.5 C42 9.5 38.5 5 32.5 5 C27.5 5 24 8.5 22 11 Z" fill="url(#am1)" transform="translate(58.16,21.3) scale(0.72)"/></svg>
          ...
          <>
            ...
            <label style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", cursor: "pointer", marginBottom: "1.25rem", marginTop: "0.25rem" }}>
              ...
              <span style={{ fontSize: "0.72rem", color: "#8C7A68", fontFamily: "'DM Sans',sans-serif", lineHeight: 1.55 }}>
                Send me occasional updates — including a reminder to retake Attune in a year, when results are most useful.
              </span>
            </label>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#1e1a35', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem 1.25rem', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '2.5rem' }}>
        <svg width="32" height="23" viewBox="0 0 103 76" fill="none"><defs><linearGradient id="plg" x1="0" y1="0" x2="103" y2="76" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#E8673A"/><stop offset="100%" stopColor="#1B5FE8"/></linearGradient></defs><path d="M14,4 L44,4 A9,9 0 0,1 53,13 L53,42 A9,9 0 0,1 44,51 L20,51 L6,61 L11,51 A6,6 0 0,1 5,45 L5,13 A9,9 0 0,1 14,4 Z" fill="url(#plg)"/><g transform="translate(13.16,11.3) scale(0.72)"><path d="M22 11 C20 8.5 16.5 5 11.5 5 C5.5 5 2 9.5 2 14.5 C2 23 11 30 22 40 C33 30 42 23 42 14.5 C42 9.5 38.5 5 32.5 5 C27.5 5 24 8.5 22 11 Z" fill="white" opacity="0.93"/></g><path d="M89,14 L59,14 A9,9 0 0,0 50,23 L50,52 A9,9 0 0,0 59,61 L83,61 L97,71 L92,61 A6,6 0 0,0 98,55 L98,23 A9,9 0 0,0 89,14 Z" fill="none" stroke="url(#plg)" strokeWidth="2.2" strokeLinejoin="round"/><g transform="translate(58.16,21.3) scale(0.72)"><path d="M22 11 C20 8.5 16.5 5 11.5 5 C5.5 5 2 9.5 2 14.5 C2 23 11 30 22 40 C33 30 42 23 42 14.5 C42 9.5 38.5 5 32.5 5 C27.5 5 24 8.5 22 11 Z" fill="url(#plg)"/></g></svg>
...
export default function App() {
  ...
  const [showAuth, setShowAuth] = useState(false); // Auth/signup modal
          ...
          <div onClick={() => setView('home')} style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer" }}>
            {(isMobile && view !== "results") ? (
              ...
              <svg width="28" height="20" viewBox="0 0 103 76" fill="none">
                ...
                <path d="M89,14 L59,14 A9,9 0 0,0 50,23 L50,52 A9,9 0 0,0 59,61 L83,61 L97,71 L92,61 A6,6 0 0,0 98,55 L98,23 A9,9 0 0,0 89,14 Z" fill="none" stroke="url(#mobileNavGrad)" strokeWidth="2.2" strokeLinejoin="round"/>
            ...
            <div style={{ width: 210, flexShrink: 0, background: "#FAF7F2", display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh", overflowY: "auto", borderRight: "1px solid #E8DDD0" }}>
              ...
              <div style={{ padding: "1.1rem 1.25rem 0.9rem", display: "flex", alignItems: "center", gap: 9, borderBottom: "1px solid #E8DDD0" }}>
                <div onClick={() => window.location.href = '/home'} style={{ display: "flex", alignItems: "center", gap: 9, cursor: "pointer" }}>
                  <svg width="28" height="20" viewBox="0 0 103 76" fill="none">
                    ...
                    <path d="M89,14 L59,14 A9,9 0 0,0 50,23 L50,52 A9,9 0 0,0 59,61 L83,61 L97,71 L92,61 A6,6 0 0,0 98,55 L98,23 A9,9 0 0,0 89,14 Z" fill="none" stroke="url(#sidebarLogoGrad)" strokeWidth="2.2" strokeLinejoin="round"/>
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 1, padding: "0.25rem 0.7rem 0" }}>
                {[
                  ...
                  { label: "Exercises", viewId: "exercise1", icon: <svg width="15" height="15" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="7.5" stroke="currentColor" strokeWidth="1.5"/><path d="M7 10l2 2 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg> },
            ...
            {ex1Answers
              ? <div style={{ textAlign: "center", padding: "4rem 1rem 3rem", maxWidth: 440, margin: "0 auto" }}>
                  ...
                  <div style={{ display: "flex", gap: "0.75rem", justifyContent: "center", flexWrap: "wrap" }}>
                    <button onClick={() => { if (window.confirm("Retaking the exercise will reset your communication answers. Your joint results will update once both partners have completed it again. Continue?")) setEx1State(null); }} style={{ background: "transparent", border: ("1.5px solid " + (C.stone)), color: C.muted, padding: "0.6rem 1.25rem", fontSize: "0.7rem", letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", fontFamily: font.body, borderRadius: 8 }}>Retake</button>
                    {bothDone
          ...
          <div>
            {ex2Answers
              ? <div style={{ textAlign: "center", padding: "4rem 1rem 3rem", maxWidth: 440, margin: "0 auto" }}>
                  ...
                  <div style={{ display: "flex", gap: "0.75rem", justifyContent: "center", flexWrap: "wrap" }}>
                    <button onClick={() => { if (window.confirm("Retaking this exercise will reset your expectations answers. Your joint results will update once both partners have completed it again. Continue?")) setEx2State(null); }} style={{ background: "transparent", border: ("1.5px solid " + (C.stone)), color: C.muted, padding: "0.6rem 1.25rem", fontSize: "0.7rem", letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", fontFamily: font.body, borderRadius: 8 }}>Retake</button>
                    {bothDone
                      ? <button onClick={() => setView("results")} style={{ background: "linear-gradient(135deg, #E8673A, #1B5FE8)", color: "white", border: "none", padding: "0.6rem 1.75rem", fontSize: "0.7rem", letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", fontFamily: font.body, borderRadius: 8, fontWeight: 600 }}>See Your Results →</button>
                      : <button onClick={() => setView("home")} style={{ background: "#2d2250", color: "white", border: "none", padding: "0.6rem 1.5rem", fontSize: "0.7rem", letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", fontFamily: font.body, borderRadius: 8 }}>Back to Dashboard →</button>
                    }
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}
          ...
          <div style={{ background: "#FFFDF9", borderRadius: 22, padding: "2rem", width: "100%", maxWidth: 440, boxShadow: "0 32px 80px rgba(0,0,0,0.28)", position: "relative", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <button onClick={() => {
              ...
              if (!name) return;
              const updated = { ...account, name, pronouns, partnerName, partnerPronouns, partnerEmail };
              setAccount(updated);
              saveAccount(updated);
