...
| # | Review |
|--|--|
| O472 | **The invite step is in the app.** Paste the invite link or type the code, and it checks before you fill anything in: it says who invited you and prefills their name, or says the code does not match, or says the invite has already been used. The link happens after the account exists, because the server checks the id it is linking has a profile row. If the link itself fails, the account is still made and the screen says so: refusing an account someone has already created would be the wrong half to undo. **On the email:** it does not tell them to download the app, and it must not yet. `APP_LIVE` is false and the store URL still ends in `idPENDING`, so an invitee sent to download it would find nothing. The line is written and switched off; flipping that one flag turns it on here, in the order email and on every page at once. |
