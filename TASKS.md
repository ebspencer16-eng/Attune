...
| # | Decision |
|--|--|
| M72 | **TestFlight: you are at the App Store Connect step.** Steps 1 to 4 of `app/TESTFLIGHT.md` are done. The two Supabase values are in EAS: `env:create` is deprecated and its replacement did not take the environment the way its prompt implied, so they went up with `eas env:push production --path .env`, which reads them out of the app's own .env and copies nothing by hand. I ran it with your session on this machine; `npx eas-cli env:list --environment production` shows both. What is left is the app record at Apple, the build, and the submit. |
