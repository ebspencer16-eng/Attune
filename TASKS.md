...
| # | Decision |
|--|--|
| D9 | **An invited partner cannot link their account from the app.** Found by sweeping the path your beta actually takes, one partner on each surface. The app has no invite step at all: it never calls `/api/partner-sync`, and its `createProfile` cannot send an invite code, so someone who is invited and installs the app first creates an unlinked account and has no way to find their partner from the phone. **It does not affect your four testers**: Aspen, Ryan, Lanae and Spencer already exist and are already linked to each other, which migration 075 checked before it would run. It affects the next person who joins by invite and reaches for the app. Two ways to go: tell any invited tester to accept the invite on the website once and then use the app for everything after, which costs nothing and is true today; or I build the invite step into the app, which is a screen, a client call and a round of testing. My view is the first for this beta and the second before anyone who is not a tester gets an invite. |
...
| # | Review |
|--|--|
| O457 | **The workbook opens the file that already exists.** The tap was trying the slowest path first: fetch the payload, pack it into a URL tens of kilobytes long, and have the browser draw a PDF from scratch, with a built file sitting unused. That is why nothing seemed to happen and then the browser opened late saying "Building your workbook". A file that exists now opens immediately, the tile dims and spins from before the first request, and the waiting page carries the mark. **What I could not fix from here:** your own workbook was never built by the trigger, because that trigger fires when a couple's results unlock and yours unlocked before it existed. Your first tap will still build one. Every couple from here on gets it built for them. |
| O458 | **The error banner.** Two separate causes, both fixed and both gated. Eleven calls that iOS can reject had nothing catching them, and an onPress does not await what it calls, so a rejection reached the screen as a red counter in development and as a tap that did nothing in a real build. And `resources.tsx` called a hook below five early returns, so opening an article, the budget, the checklist or a shelf rendered one hook fewer than the render before it and React threw over the whole screen. The check written for the second found a worse one: every paragraph in the results returned early when it had no text, above five hooks. |
| O465 | **Both couples reset, and the migration is M075 in section 1.** Read its header before running it: it says what the earlier resets left behind and why each one showed up later as something that looked like a bug. The short version is that results are frozen on purpose, so a reset that clears the answers and leaves `couple_results` shows last month's results next to empty answers, and nothing in the product can tell that is wrong. |
| O466 | **Three sweeps so far, each with a method rather than a look.** Every deployed endpoint from outside: fifty-eight of them, no 5xx, everything answering 200, 400, 401 or 405. The whole website rendered: twenty-seven sections, thirteen views, thirty-seven static pages, and three exercises driven to completion and stored. And two new standing checks over the app, which between them found four real crashes or dead taps that nobody had reported. More rounds to come. |
| O446 | **A tap anywhere clears a selection, and the ✕ is gone.** The ✕ existed because nothing else dismissed one: every paragraph is its own selector, and a tap on the page belongs to whatever is under it, which is usually nothing. The provider watches every touch that begins in the results now. It also fixed something nobody reported: selecting in one paragraph used to leave the previous paragraph's selection painted, because neither knew about the other. |
| O448 | **Text on the results pages is selectable.** The rule that decides this covered one type size, and the overview pages carry their findings at a smaller one and their headings at two larger ones, so it reached none of them and the check was green about a page where nothing could be selected. It covers five sizes now. Forty-one places became selectable; forty-six are exempted one at a time, each with a written reason, because a rank number, a dash, a caret, a count, a person's name and a truncated row are not findings. The nav text you excluded is one of them. |
| O450 | **Save the insight of the day to your journal.** A control on the insight's own page and one beside Share on the Learn banner. Both open a sheet with the quote in it and a field for what you want to say about it. The quote is kept in the column that already means "the words this was made on", so it stays separable from your own writing months later, and both surfaces draw the tile the same way. **Two things I could not verify by eye:** the sheet, and the saved tile. The simulator stops accepting ordinary presses after a run of hot reloads, which is written up in CLAUDE.md, and it did it four times while I was trying. What is proved: the server accepts the write and keeps both halves (a gate posts one and reads the row it wrote), and both surfaces draw the quote. What is not: how the sheet looks and feels. |
...
| O447 | **The hold before a selection starts is shorter**, 450 to 320. This is a partial fix and I want to be straight about it: the old number was chosen against a synthetic press, which is perfectly still for exactly as long as it is asked to be, and a thumb is not. Shortening the wait shortens the window in which a wobble can be read as a scroll. What it does not do is widen how far the thumb may move, which is the knob iOS uses for its own long press and the one this cannot set from JavaScript. **It cannot be judged in a simulator**, because a synthetic press never wobbles. If it is still finicky on your phone, say so and the next step is a gesture relation with the scroll view, which I would want to measure on a device rather than guess at. |
| O449 | **Two fewer round trips before a page draws.** Measured rather than guessed. All four tabs mount when the app starts and three of them ask /api/home, so that was three identical requests for one answer before any of them could draw. Calls that overlap now share one. And Insights asked /api/home, waited, and only then asked /api/results: two serial trips before the slowest page in the product could show anything, on a request whose answer it also contains. They go together now. **Judge this on a build rather than on my description**, and if it is still slow the next thing to measure is what the app does between the answer arriving and the page appearing, which is a different problem with a different fix. |
| O440 | **Tapping Learn brings you back to Learn.** It did nothing from inside a shelf page, because the hook that does this had been told about the article reader and the tools and never about the shelf. And the sheet is not a page you leave, it is the bottom of the tab's own scroll, so coming back also means scrolling back to the top. Verified both ways in the simulator: from a shelf, and from an open article. |
| O442 | **The scan, and what it turned up.** Four things, and only two of them were bugs. **Fixed:** every address the site publishes to a crawler pointed at the apex rather than the www, so all twenty sitemap entries, robots.txt's own sitemap line and the canonical, og:url and og:image tags on eleven pages were URLs that answer 307. Sixty-four of them. That is the same "keep the www" rule the repo already enforces on outbound email and nowhere else, and it is gated now. **Also fixed:** the gate meant to stop a second copy of the site address had two branches that disagreed with each other, and which one applied depended on how the URL was typed; the JSON-LD blocks in home.html were excused by one branch and invisible to the other. **Verified rather than assumed:** I pointed the public key at ten tables and every one answered with an empty list, then tried to write to four and every one was refused by row-level security, with a body that could not have created a row either way. Every admin endpoint answers 401 or 405 and none answers 500, which is the thing that was wrong the last time this was swept. The three URLs that 404 are the pages you retired, deliberately, in 1316f37. **For you to decide:** below. |
...
| O452 | **No white outline on the mark on Learn.** It stays on the Insights orange, where you asked for it and where the bubble's own orange dissolves into the page. Three generated files from one source SVG rather than a second logo. |
| O433 | **An orange grab line over the In Practice sheet.** The peek's height came down by exactly the line's height plus its margin, so the same four articles and the same search bar are above the fold as before. |
| O436 | **The four labels start at the page's edge.** They were indented past a column that held an arrow until one round ago. What they line up with has changed on purpose: the page's left edge rather than the exercises' icons, which is what the heading over them is for. |
| O437 | **Workbook, budget, checklist**, and the checklist's tile now says what you called it. Reordered in the catalogue itself rather than sorted in the app, so every surface that lists the three lists them this way. |
| O438 | **The shelf page.** White hero, the Saved and Read pills counting that shelf rather than the whole feed, the sort menu, and the articles as a table: name on the left, read time on the right. |
| O439 | **Filter and sort under the caret**, so neither is on the peek. The filter narrows to one shelf. |
...
| O434 | **Journal entries are italic**, in the app and on the website. |
...
| A reset migration clears every exercise, the completion flags and the frozen results | `check-reset-migrations.mjs`, planted four ways, with the column list derived from api/_exercises.js |
| The checklist has one name | `check-checklist-name.mjs`. The old name is matched as a pairing rather than as a phrase, so the Starting Out Collection is untouched |
| A question set out as an A-to-B scale is one on both surfaces | `check-ab-scale.mjs`, planted nine ways. Two of those passed at first: disabling a branch with a constant left the poles in the source and the gate read them, so the guard is matched as well as the draw |
| The insight list and its review document cannot disagree | `check-insights-review.mjs` |
| The word list and the review document cannot disagree | `check-words-review.mjs`, planted four ways. WORDS-REVIEW.md is regenerated and compared rather than read, which is the only check a partial edit cannot fool, and nothing in the generator depends on the date, so it can never fail for a reason that is not a reason |
| An Expectations page can be reordered and cannot be renamed | `check-expectations-order.mjs`, planted six ways, including one that has to be ALLOWED: changing the order back must pass, because the order is Ellie's and is expected to change. That is the whole reason the id was taken off it |
| The overview's action tiles say the same thing on both surfaces | `check-action-tile-fields.mjs`, planted five ways. It runs the builder over a fixture and holds every field it emits to being drawn by both renderers, and both renderers to drawing nothing the builder does not send. The prompt on "When things get hard" was a field on one of three domains, and the two surfaces agreed with each other while neither agreed with the page behind the tile |
| Every address the site publishes keeps the www | `check-published-urls.mjs`, planted six ways. Found by O442: all twenty URLs in sitemap.xml, robots.txt's own sitemap line, and the canonical, og:url and og:image on eleven static pages named the apex, which answers 307. Sixty-four addresses, every one of them redirecting. The host comes from SITE_URL rather than being typed into the check |
