...
| # | Review |
|--|--|
| R173 | **The workbook opens as the PDF the website prints.** Not the page scaled down, and not a second renderer: api/workbook-pdf.js prints public/workbook-render.html with headless Chrome and the website's own print settings. Zoom, share and print are the phone's. Two things to look at: the cover runs a little onto a second page, which is the page's own layout rather than the printing, and the site has never had its server renderer configured, so its own PDF has always been built in the browser. If you want the cover to fill the sheet, that is a change to the page and it changes the website's PDF too. |
