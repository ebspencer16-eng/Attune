...
| # | Decision |
|--|--|
| D7 | **Old website notes are stored and shown nowhere.** From O442. Before the Notes page was rebuilt against /api/notes, the website kept notes in a `notes_data` column on your profile: three boxes, Shared, yours and your partner's, labelled "auto-saved". That column is still read on every sign-in and written into the browser, and the screen that displayed it is still in the code and is never opened. So nothing is lost and nothing is reachable. Three ways to go: leave it, which is what is happening now; surface it on the Notes page as an older block, which needs a line of copy from you; or delete the column, which is a migration and cannot be undone. My view is the middle one if anyone actually wrote in those boxes and the last if nobody did, and I can tell you which by counting non-empty rows before you decide. |
| D8 | **Eight components in the app and the site are defined and never drawn.** From O442, by grepping each name across the whole of src/ and attune-app/src/ and finding exactly one hit, which is its own declaration: `PrimaryCard` and `ResearchNote` on the home screen, `Bullet` in Settings, `Primary` and `Secondary` in two exercises, and `ExpectationRowView`, `DistanceBar` and `Legend` in results. None of them is a bug. They are what is left when a design changes, and each one is a thing a future session can mistake for the live version of what it draws. Say the word and they go in one commit. |
